...
pub(crate) fn normalize_ws(mut s: String) -> String {
    s = s.replace('_', " ");
...
pub(crate) fn strip_year_prefix(mut s: String) -> String {
...
pub(crate) fn strip_year_suffix(mut s: String) -> String {
    let re = Regex::new(r"(?i)\s*[\(\[]\s*\d{4}\s*[\)\]]\s*$").unwrap();
    s = re.replace(&s, "").to_string();
    s.trim().to_string()
}

pub(crate) fn strip_tech_brackets(mut s: String) -> String {
    let re = Regex::new(
...
pub(crate) fn strip_catalog_suffix(mut s: String) -> String {
...
pub(crate) fn strip_artist_prefix(mut album: String, artist: &str) -> String {
    for sep in [" - ", " – ", " — "] {
